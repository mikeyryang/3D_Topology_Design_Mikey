...
if __name__ == "__main__":
    # Create solver
    solver = HexFEMSolver3D(E_mod=200e9, nu=0.3)
    
    # Create mesh
    solver.set_mesh(Lx=1.0, Ly=0.1, Lz=0.1, nx=10, ny=3, nz=3)
    
    # Fix left face
    solver.fix_face(axis=0, coord=0.0)
    
    # Apply load on right face (downward in y direction)
    solver.add_distributed_load(axis=0, coord=1.0, direction=1, total=-1e4)
    
    # Solve
    result = solver.solve()
    
    print(f"Total compliance: {result['compliance']:.6f}")
    print(f"Max displacement: {np.max(np.abs(result['u'])):.6e}")